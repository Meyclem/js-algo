...
describe(`#squareNumbers`, () => {
    const filePath = path.resolve(__dirname, `../src/08-magic-square.js`);
    let squareNumbers;
    ...
    test(`'Array.Prototype.map()' est utilisé`, async () => {
        const code = await readCode(filePath);

        // 🤖 Il faut utiliser '.map' sur 'numbers'
        expect(code).toMatch(/\.map\(/g);
    });
